	...
	class rcou_widget extends wp_widget{
		...
		function widget($arguments, $data){
			...
			$defaut = array(
				...
				"saltrc" => "print(get_option( 'saltrc', '' ));",
				);
				...
				?>
					<div class="rcwidget">
						...
						<div class="rconplayer">
							...
							<p><?php print_r($PlayerCount["success"]) ?><strong> /<?php print_r($PlayerLimit["success"]) ?></strong></p>
							<div><?php 
									if ($PlayerNames["success"] == '') { 
										echo "";
									}
									else {
		...
		function update($content_new, $content_old){
			...
			$content_new['saltrc'] = esc_attr($content_new['saltrc']);
			return $content_new;
		...
		function form($data){
			...
			<p>
				<!-- JSONAPI's Salt -->
				<label for="<?php echo $this->get_field_id('saltrc'); ?>">Votre salt</label><br />
				<input value="<?php print(get_option( 'saltrc', '' )); ?>" name="<?php echo $this->get_field_name('saltrc'); ?>" id="<?php echo $this->get_field_id('saltrc'); ?>" type="text" />
			</p>
			<?php
